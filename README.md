...
#### Table 2 の E_min と高エネルギー透過時の損失 ΔE の違い

Table 2 の E_min（例: 100 μm で 0.595 MeV）は、**垂直入射でその厚さをギリギリ抜けるために必要な最小の入射エネルギー**であり、R(E_min) = t で与えられる。これは「その厚さ分の飛程に相当する入射エネルギー」であり、**高エネルギー粒子が同じ厚さを通過するときに失うエネルギー ΔE = E − ε とは別物**である。

誤解しやすいのは、「ガラス厚 t に対応する飛程のエネルギー分だけ常に損失する」と考えることである。飛程 R(E) は粒子が停止するまでの累積飛行距離を表す。式 (2) は、パス長分だけ**残り飛程**が減る、すなわち R(ε) = R(E) − t/cosθ と読む。したがって損失は ΔE = E − ε であり、一般に「R(ΔE) = t を満たす ΔE」とは一致しない（後者は低エネルギー域で「その厚さでほぼ止まる」スケールのエネルギーに近い）。

**5 MeV・100 μm・垂直入射での約 0.14 MeV**は、式 (1) の R(E) に式 (2)（θ = 0）を適用し、R(ε) = R(5 MeV) − 100 μm を満たす残留エネルギー ε を数値的に求めたうえで ΔE = 5 − ε とした結果である（上記のフィット係数では R(5 MeV) ≈ 2439 μm、R(ε) ≈ 2339 μm に対応する ε ≈ 4.86 MeV、よって ΔE ≈ 0.14 MeV）。実装では `src/proton_glass_transmission.py` の `range_func` および飛程からエネルギーを逆算する `find_energy_from_range`（Brent 法）と同じ飛程モデルに基づく。

**物理的イメージ（飛程とガラス厚の比較）:** 入射陽子の飛程がカバーガラス厚と**同程度**であれば、材内で飛程に近い距離まで進むため**多くのエネルギーを失い**、透過できても出口付近でエネルギーはほとんど残らない（Table 2 の E_min 付近の挙動）。一方、飛程が厚みに比べて**十分大きい**高エネルギー陽子では、同じ幾何学的パス長でも**残り飛程のうち占める割合が小さい**ため、通過に伴う損失 ΔE は前者より**はるかに小さく**なる。式 (2) の「パス長ぶんだけ残り飛程 R(ε) が減る」という記述は、この対比を厳密に表したものである。
